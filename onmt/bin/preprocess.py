...
def process_one_shard(corpus_params, params):
    corpus_type, fields, src_reader, tt_reader, tgt_reader, align_reader, opt,\
         existing_fields, src_vocab, tt_vocab, tgt_vocab = corpus_params
    i, (src_shard, tt_shard, tgt_shard, align_shard, maybe_id, filter_pred) = params
    ...
    ####################=====================================================
    tt_data = {"reader": tt_reader, "data": tt_shard, "dir": None}
    ...
    _readers, _data, _dir = inputters.Dataset.config(
        [('src', src_data), ('tt', tt_data),('tgt', tgt_data), ('align', align_data)])
    # for k,v in fields.items():
    #     if(k in ['src','tgt','tt']):
    #         print(("preprocess .py process_one_shard() fields_item",k,v.fields[0][1].include_lengths))
    ...
    #     print((name,field,len))
    # print((type(dataset),dataset.fields['tt'][0][1].include_lengths,type(dataset.examples)))
...
def build_save_dataset(corpus_type, fields, src_reader, tt_reader, tgt_reader,
                       align_reader, opt):
    ...
    #############add tt_vocab=================================
    tt_vocab = None
    # print(("preprocess.py build_save_dataset()",src_vocab,tgt_vocab,existing_fields))
    ...
    def shard_iterator(srcs, tts, tgts, ids, aligns, existing_shards,
        ...
        # print(tts)
        # exit()
        for src, tt, tgt, maybe_id, maybe_align in zip(srcs, tts, tgts, ids, aligns):
            ...
            #############==================================================
            tt_shards = split_corpus(tt, opt.shard_size)

            tgt_shards = split_corpus(tgt, opt.shard_size)
            align_shards = split_corpus(maybe_align, opt.shard_size)
            ###################==============================================
            for i, (ss, tms, ts, a_s) in enumerate(
                    zip(src_shards, tt_shards,tgt_shards, align_shards)):
                # print(ss)
                # print(tms)
                # print(ts)
                # exit()
                yield (i, (ss, tms, ts, a_s, maybe_id, filter_pred))

    shard_iter = shard_iterator(srcs, tts, tgts, ids, aligns, existing_shards,
                                existing_fields, corpus_type, opt)
    # for k,v in fields.items():
    #     if(k in ['src','tgt','tt']):
    #         print(("preprocess.py build_save_dataset() fields_item",k,v.fields[0][1].include_lengths))

    with Pool(opt.num_threads) as p:
        ########################add tt_vocab in dataset_params=====================
        dataset_params = (corpus_type, fields, src_reader, tt_reader, tgt_reader,
                          align_reader, opt, existing_fields,
                          src_vocab, tt_vocab, tgt_vocab)
...
def preprocess(opt):
    ...
    build_save_dataset(
        'train', fields, src_reader, tt_reader, tgt_reader, align_reader, opt)
